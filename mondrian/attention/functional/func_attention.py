...
def func_attention(query, key, value, quadrature_weights, bias=None, shifted=False, return_scores=False):
    if quadrature_weights is None:
        quadrature_weights = 1
    if bias is not None:
        return naive_func_attention(query, key, value, quadrature_weights, bias=bias, shifted=shifted)
