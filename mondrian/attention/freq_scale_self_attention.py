# TODO: This file is basically a copy-paste of func_self_attention.py
# they can easily be combined, but I am too lazy to do that right now.
# This seemed to not be an improvement over normal attention, which
# is not super surprising. None of the models support using this ATM.
# This can be added just be replacing `FuncSelfAttention` in the vit
# operator.
...
from .functional.freq_scale_attention import freq_scale_attention
...
class FreqScaleSelfAttention(nn.Module):
    def __init__(
        self,
        embed_dim: int,
        channel_heads: int,
        ...
        self.attn_scale = nn.Parameter(torch.zeros(num_heads))
    ...
    def _forward_spatial_heads(self, seq, n_sub_x, n_sub_y):
        ...
        sa = freq_scale_attention(
            query, key, value, self.attn_scale, attn_mask
        )
