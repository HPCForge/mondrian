...
def apply_3d_rotary_pos_emb(t, freqs_x, freqs_y, freqs_z):
    ...
    return torch.cat(
        (
            ...
            apply_rotary_pos_emb(t_z, freqs_z),
        ),
        dim=-1,
    )
...
class GaussianFourierFeatureTransform(nn.Module):
    ...
    def __init__(
        self, num_input_channels, mapping_size=256, scale=10, learnable=False, num_heads=1
    ):
        super().__init__()
        ...
        self._B = nn.Parameter(
            torch.randn((num_input_channels, mapping_size * num_heads)) * scale,
