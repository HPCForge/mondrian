...
class PoolingReducer(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim):
        ...
        self.out_ffn = PreNorm(
            in_dim, MLP([hidden_dim, hidden_dim, out_dim], GeAct(nn.GELU()))
        )
...
class FABlock2D(nn.Module):
    ...
    def __init__(
        ...
        use_softmax = False
        self.low_rank_kernel_x = LowRankKernel(
            self.latent_dim,
            ...
            softmax=use_softmax,
            scaling=(
                1 / np.sqrt(dim_head * kernel_multiplier)
                if kernel_multiplier > 4 or use_softmax
                else scaling_factor
            ),
        )
        self.low_rank_kernel_y = LowRankKernel(
            self.latent_dim,
            dim_head * kernel_multiplier,
            heads,
            positional_embedding=positional_encoding,
...
class FABlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.low_rank_kernel_z = LowRankKernel(
            self.latent_dim,
            dim_head * kernel_multiplier,
            heads,
            positional_embedding=positional_encoding,
