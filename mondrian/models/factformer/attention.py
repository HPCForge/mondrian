...
from .positional_encoding_module import (
    RotaryEmbedding,
...
class AsymetricKernel(nn.Module):
    ...
    def __init__(
        ...
        if positional_embedding == "rff":
            self.pos_emb = GaussianFourierFeatureTransform(
                pos_dim, self.kernel_dim, scale=8, learnable=False, num_heads=heads
            )
        elif positional_embedding == "rotary":
            self.pos_emb = RotaryEmbedding(
                self.kernel_dim // self.pos_dim, min_freq=1 / 64
            )
        elif positional_embedding == "learnable":
            self.pos_emb = nn.Sequential(
                GaussianFourierFeatureTransform(
                    pos_dim, dim_head * heads // 2, scale=8, learnable=True
                ),
        ...
        for param in weight.parameters():
            if param.ndim > 1:
                ...
                for h in range(self.heads):
                    inif_fn(
                        param[h * dim_head : (h + 1) * dim_head, :], gain=self.init_gain
                    )
                    #
    ...
    def normalize_wrt_domain(self, x, norm_fn):
        b = x.shape[0]
        return rearrange(
            norm_fn(rearrange(x, "b h n d -> (b h) n d")), "(b h) n d -> b h n d", b=b
        )

    def forward(
        self, u_x, u_y=None, pos_x=None, pos_y=None, associate=True, get_attention=False
    ):
        # u_x, u_y: b n c
        ...
        if (
            ...
            and self.positional_embedding != "none"
            and self.positional_embedding != "rff"
        ):
            ...
            if self.pos_dim == 3:
                ...
                q_freqs_x = repeat(
                    q_freqs_x, "1 n d -> b h n d", b=q.shape[0], h=q.shape[1]
                )
                q_freqs_y = repeat(
                    q_freqs_y, "1 n d -> b h n d", b=q.shape[0], h=q.shape[1]
                )
                q_freqs_z = repeat(
                    q_freqs_z, "1 n d -> b h n d", b=q.shape[0], h=q.shape[1]
                )
                ...
                if pos_y is None:
                    ...
                    k_freqs_x = repeat(
                        k_freqs_x, "1 n d -> b h n d", b=q.shape[0], h=k.shape[1]
                    )
                    k_freqs_y = repeat(
                        k_freqs_y, "1 n d -> b h n d", b=q.shape[0], h=k.shape[1]
                    )
                    k_freqs_z = repeat(
                        k_freqs_z, "1 n d -> b h n d", b=q.shape[0], h=k.shape[1]
                    )
                ...
                if pos_y is None:
                    ...
                    k_freqs = repeat(
                        k_freqs, "1 n d -> b h n d", b=q.shape[0], h=q.shape[1]
                    )
...
class LowRankKernel(nn.Module):
    ...
    def __init__(
        ...
        if positional_embedding == "rff":
            self.pos_emb = GaussianFourierFeatureTransform(
                pos_dim, dim_head, scale=1, learnable=False, num_heads=heads
            )
        elif positional_embedding == "rotary":
            ...
            self.pos_emb = nn.Sequential(
                GaussianFourierFeatureTransform(
                    pos_dim, dim_head * heads // 2, scale=1, learnable=True
                ),
                nn.Linear(dim_head * heads // 2, dim_head * heads, bias=False),
                nn.GELU(),
        ...
        if self.residual:
            self.gamma = nn.Parameter(
                torch.tensor(1 / np.sqrt(dim_head)), requires_grad=True
            )
        else:
            self.gamma = 0
    ...
    def forward(self, u_x, u_y=None, pos_x=None, pos_y=None):
        ...
        K = (
            torch.einsum("bhid,bhjd->bhij", q, k) * self.scaling
        )  # if not on uniform grid, need to consider quadrature weights
        K = self.dropout(K)
        if self.softmax:
            K = F.softmax(K, dim=-1)
        if self.residual:
