...
from mondrian.grid.vit_self_attention_operator import WinSelfAttentionOperator
...
class Embed(nn.Module):
  def __init__(self, 
               ...
               window_size):
    super().__init__()
    self.sa = WinSelfAttentionOperator(embed_dim, num_heads, window_size)
    modes = (16, 16)
...
class WinSAOperator2d(nn.Module):
  ...
  def __init__(self,
    ...
    self.embed = nn.ModuleList([
      Embed(embed_dim, num_heads, window_size) for _ in range(num_layers)
    ])
