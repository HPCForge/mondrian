...
class LogCPB(nn.Module):
  ...
  def _get_grid(self, n_sub_x, n_sub_y, device):
    ...
    """
    relative_coords_h = torch.arange(-n_sub_y // 2, n_sub_y // 2, dtype=torch.float32, device=device)
    relative_coords_w = torch.arange(-n_sub_x // 2, n_sub_x // 2, dtype=torch.float32, device=device)
    relative_coords_table = torch.stack(
        torch.meshgrid([relative_coords_h,
                        relative_coords_w], indexing='ij')).contiguous().permute(1, 2, 0)
