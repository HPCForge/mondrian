...
class RandomProjectionConv2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.up_x_func = nn.Parameter(torch.randn(self.order, self.inner_dim), requires_grad=False)
        self.up_y_func = nn.Parameter(torch.randn(self.order, self.inner_dim), requires_grad=False)
        
    def _get_down_x(self, res):
        return F.interpolate(self.down_x_func.unsqueeze(0).unsqueeze(0), size=(res, self.inner_dim), mode='bilinear').squeeze()
        
    def _get_down_y(self, res):
        return F.interpolate(self.down_y_func.unsqueeze(0).unsqueeze(0), size=(res, self.inner_dim), mode='bilinear').squeeze()
    
    def _get_up_x(self, res):
        return F.interpolate(self.up_x_func.unsqueeze(0).unsqueeze(0), size=(res, self.inner_dim), mode='bilinear').squeeze()
    
    def _get_up_y(self, res):
        return F.interpolate(self.up_y_func.unsqueeze(0).unsqueeze(0), size=(res, self.inner_dim), mode='bilinear').squeeze()
