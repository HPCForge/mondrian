...
from mondrian.constants import (
  REIMANN, TRAPEZOID, SIMPSON_13
)
...
def reimann_quadrature_weights(
  ...
  deltas = torch.tensor([
    grid_axis_spatial / (grid_axis_disc - 1) for (grid_axis_spatial, grid_axis_disc) in zip(grid_spatial_dimensions, grid_discretization_dimensions)
  ])
...
def get_quadrature_weights(
  ...
  return _quadrature_lookup[method](grid_spatial_dimensions, grid_discretization_dimensions)
...
class Quadrature2d(nn.Module):
  ...
  def forward(self, x):
    ...
    quadrature_weights = get_quadrature_weights(self.method,
                                                (1, 1),
                                                (x.size(-2), x.size(-1))).to(x)
