r"""
Taken and modified from https://github.com/aditya-grover/climate-learn/.
  """
...
def visualize_at_index(
    ...
    if src == "era5":
        variable_with_units = f"{out_variable} ({ERA5_VAR_TO_UNIT[variable_type]})"
    elif src == "cmip6":
        variable_with_units = f"{out_variable} ({CMIP6_VAR_TO_UNIT[variable_type]})"
    elif src == "prism":
    ...
    for batch in tqdm(dm.test_dataloader()):
        ...
        if index in range(counter, counter + batch_size):
            ...
            x = x.to(mm.device)
            x_flat = x_flat.to(mm.device)
            pred = mm.forward(x_flat)
            break
        counter += batch_size
