...
class ShearLayerDataset(Dataset):
    def __init__(
        ...
        if in_dist:
            if self.s == 64:
                self.file_data = (
                    f"{data}/NavierStokes_64x64_IN.h5"  # In-distribution file 64x64
                )
            else:
                self.file_data = (
                    f"{data}/NavierStokes_128x128_IN.h5"  # In-distribution file 128x128
                )
        else:
            self.file_data = (
                f"{data}/NavierStokes_128x128_OUT.h5"  # Out-of_-distribution file 128x128
            )
    ...
    def __getitem__(self, index):
        ...
        if self.s == 64 and self.in_dist:
            ...
            inputs = self.reader["Sample_" + str(index + self.start)]["input"][:].reshape(
                1, 1, self.s, self.s
            )
