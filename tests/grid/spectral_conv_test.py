...
def test_spectral():
  ...
  for o in operators:
    spectral = SimpleSpectralConv2d(in_channels=1, out_channels=1, n_modes=32, version=o)
    coarse = spectral(get_func(100)).squeeze(1)
    fine = spectral(get_func(400)).squeeze(1)        
    coarse_interp = interpolate(coarse, size=(fine.size(-2), fine.size(-1)), mode='bicubic')
  ...
  spectral = SimpleSpectralConv2d(in_channels=1, out_channels=1, n_modes=32, version='cnn')    
  coarse = spectral(get_func(100)).squeeze(1)
  fine = spectral(get_func(400)).squeeze(1)   
  coarse_interp = interpolate(coarse, size=(fine.size(-2), fine.size(-1)), mode='bicubic')
  assert ((coarse_interp - fine) ** 2).mean() > 1e-3