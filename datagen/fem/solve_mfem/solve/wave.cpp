...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&dirichlet, "-dir", "--dirichlet", "-neu",
                  "--neumann",
                  "BC switch.");
   args.AddOption(&log_step, "-l", "--log-step",
                  "number of steps between writing solution.");
   args.Parse();
   if (!args.Good())
   {
   ...
   if (std::string(mesh_file).empty()) {
      std::cout << "Must specify mesh_file: -m <path>";
      return 0;
   }
